...
public sealed record AdapterPaths(string CodexState, string CodexHistory, string CodexIndex, string CodexSessions, string ClaudeRoot, string CopilotStorage)
{
    public static AdapterPaths Discover(string? home = null, string? platform = null)
    {
        ...
        var appData = platform switch { "windows" => Environment.GetEnvironmentVariable("APPDATA") ?? Path.Combine(home, "AppData", "Roaming"),
            "macos" => Path.Combine(home, "Library", "Application Support"), _ => Environment.GetEnvironmentVariable("XDG_CONFIG_HOME") ?? Path.Combine(home, ".config") };
        return new(Environment.GetEnvironmentVariable("CODEX_STATE_DB") ?? Path.Combine(codex, "state_5.sqlite"),
            ...
            Environment.GetEnvironmentVariable("COPILOT_STORAGE") ?? Path.Combine(appData, "Code", "User", "globalStorage", "github.copilot-chat"));
    }
